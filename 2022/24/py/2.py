...
# search shortest path from sx, sy to tx, ty. returns the required time
def search(sx: int, sy: int, st: int, tx: int, ty: int):
    ...
    while q:
        ...
        for dx, dy in [(1,0), (0,1), (-1,0), (0,-1), (0,0)]:
            if (t+1, px+dx, py+dy) in seen: continue
            if (px,py)==(1,0) and dy == -1: continue
            if (px,py)==(w-2,h-1) and dy == 1: continue
            if states[t][py+dy][px+dx] == 1: continue
            if abs(tx-px-dx)+abs(ty-py-dy) == 1:
                return t+2
            
            seen.add((t+1, px+dx, py+dy))
            q.append((t+1, px+dx, py+dy))
    print("failed")
    return None
...
t1 = search(1,0,0, w-2, h-1)
t2 = search(w-2, h-1, t1, 1, 0)
result = search(1, 0, t2, w-2, h-1)
